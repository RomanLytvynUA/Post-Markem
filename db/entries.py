...
def create_entry(category_id, number, place, partner1_id, partner2_id=None, metadata=None):
    return execute_write("INSERT INTO entries (category_id, number, place, partner1_id, partner2_id, metadata) VALUES (?, ?, ?, ?, ?, ?)", 
                         (category_id, number, place, partner1_id, partner2_id, metadata))
...
def get_round_entries(round_id):
    query = """
        SELECT e.number, e.place, p1.name as p1_name, p2.name as p2_name
        FROM entries e
...
def get_entries_by_person(person_id):
    query = """
        SELECT * FROM entries WHERE person_id = ?
    """
